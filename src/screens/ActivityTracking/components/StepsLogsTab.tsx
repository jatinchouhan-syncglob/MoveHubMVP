...
// 5. Custom Chart Constants & Helpers
const chartWidth = Dimensions.get('window').width - 32;

const speedometerColor = {
  light: '#EF4444',     // Bronze/High Risk
  base: '#F59E0B',      // Silver/Moderate Risk
  light100: '#10B981',  // Gold/Low Risk
  medium: '#3B82F6',    // Platinum/Very Low Risk
};
...
const dailyStepsData = {
  values: [8420, 7150, 9200, 4850, 10250, 8900, 9400],
  labels: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
};

const dailyHeartPointsData = {
  values: [45, 30, 65, 10, 80, 50, 70],
  labels: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
};

const energyExpendedData = {
  values: [320, 280, 350, 190, 410, 330, 360],
  labels: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
};

const sdexActivityData = {
  values: [25, 40, 55, 15, 60, 30, 45],
  labels: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
};

const stepsChartsMeta = { target: '10,000', actual: '8,309', performance: 83 };
const heartPointsChartsMeta = { target: '150', actual: '350', performance: 233 };
const energyChartsMeta = { target: '300', actual: '320', performance: 106 };
const sdexChartsMeta = { target: '40', actual: '42', performance: 105 };

const getDynamicMax = (values: number[]) => {
  const max = Math.max(...values, 0);
  if (max === 0) return 5;
  if (max <= 10) return max + 5;
  if (max <= 100) return Math.ceil((max + 20) / 10) * 10;
  if (max <= 1000) return Math.ceil((max + 100) / 50) * 50;
  if (max <= 10000) return Math.ceil((max + 500) / 100) * 100;
...
const EmptyChart = ({ title }: { title: string }) => (
  <View style={fitnessStyles.emptyChartContainer}>
    <View style={fitnessStyles.emptyChartIconContainer}>
      <Text style={fitnessStyles.emptyChartIcon}>📊</Text>
    </View>
    <Text style={fitnessStyles.emptyChartTitle}>No {title} data available</Text>
  </View>
);
...
const ScrollableChart = ({
  ...
  return (
    <View style={{ width: visibleWidth }}>
      {dataLength > 7 && (
        <View style={fitnessStyles.scrollIndicatorContainer}>
          <Text style={fitnessStyles.scrollIndicatorText}>Scroll for more</Text>
          <Text style={fitnessStyles.scrollIndicatorArrow}>→</Text>
        </View>
      )}
      <ScrollView
        horizontal
...
// 6. Refactored Sub-tab components using custom charts
const WeeklyHealthReview: React.FC = () => {
  return (
    <>
      <View style={[fitnessStyles.card, { gap: 5 }]}>
        <View style={fitnessStyles.cardTitleRow}>
          <View style={[fitnessStyles.cardIconContainer, { backgroundColor: DC.purpleLight }]}>
            <Text style={fitnessStyles.emptyChartIcon}>📊</Text>
          </View>
          <Text style={fitnessStyles.cardTitle}>Weekly Performance</Text>
        </View>

        <View style={fitnessStyles.speedometerRow}>
          <View style={fitnessStyles.speedometerCard}>
            <Text style={fitnessStyles.speedometerLabel}>
              Heart Points{"\n"}(Weekly Total)
            </Text>
            <SegmentedGauge
              value={350}
            ...
            />
            <Text style={fitnessStyles.speedometerValueLabel}>350</Text>
            <View style={fitnessStyles.legendContainer}>
              <View style={fitnessStyles.legendRows}>
                <View style={[fitnessStyles.legendDot, { backgroundColor: speedometerColor.light }]} />
                <Text style={fitnessStyles.legendText}>0-74 Bronze / High Risk</Text>
              </View>
              <View style={fitnessStyles.legendRows}>
                <View style={[fitnessStyles.legendDot, { backgroundColor: speedometerColor.base }]} />
                <Text style={fitnessStyles.legendText}>75-159 Silver / Mod Risk</Text>
              </View>
              <View style={fitnessStyles.legendRows}>
                <View style={[fitnessStyles.legendDot, { backgroundColor: speedometerColor.light100 }]} />
                <Text style={fitnessStyles.legendText}>160-299 Gold / Low Risk</Text>
              </View>
              <View style={fitnessStyles.legendRows}>
                <View style={[fitnessStyles.legendDot, { backgroundColor: speedometerColor.medium }]} />
                <Text style={fitnessStyles.legendText}>300+ Platinum / Very Low</Text>
              </View>
            </View>
          </View>
          ...
          <View style={fitnessStyles.speedometerCard}>
            ...
            />
            <Text style={fitnessStyles.speedometerValueLabel}>42</Text>
            <View style={fitnessStyles.legendContainer}>
              <View style={fitnessStyles.legendRows}>
                <View style={[fitnessStyles.legendDot, { backgroundColor: speedometerColor.light100 }]} />
                <Text style={fitnessStyles.legendText}>{"<40: Low Risk\nHigh Cardio Mobility"}</Text>
              </View>
              <View style={fitnessStyles.legendRows}>
                <View style={[fitnessStyles.legendDot, { backgroundColor: speedometerColor.base }]} />
                <Text style={fitnessStyles.legendText}>{"40-65: Moderate Risk\nBorderline Profile"}</Text>
              </View>
              <View style={fitnessStyles.legendRows}>
                <View style={[fitnessStyles.legendDot, { backgroundColor: speedometerColor.light }]} />
                <Text style={fitnessStyles.legendText}>{">65: High Risk\nArterial Loading"}</Text>
              </View>
            </View>
          </View>
        </View>
      ...
      <View style={fitnessStyles.card}>
        <Text style={fitnessStyles.cardTitle}>Insights & Alerts</Text>
        <View style={fitnessStyles.insightContainer}>
          <View style={fitnessStyles.insightItem}>
            <Text style={[fitnessStyles.insightText, { color: DC.skyBrand }]}>
              Your activity insights and personalized health alerts will appear here based on your trends.
            </Text>
          </View>
        </View>
      </View>
...
const WeeklyFitnessDashboard: React.FC = () => {
  const stepsMax = getDynamicMax(dailyStepsData.values);
  const heartPointsMax = getDynamicMax(dailyHeartPointsData.values);
  const sdexMax = getDynamicMax(sdexActivityData.values);
  ...
  return (
    <>
      ...
      <View style={fitnessStyles.card}>
        <View style={fitnessStyles.cardTitleRow}>
          <View style={[fitnessStyles.cardIconContainer, { backgroundColor: DC.blueLight }]}>
            <Text style={fitnessStyles.emptyChartIcon}>👣</Text>
          </View>
          <Text style={fitnessStyles.cardTitle}>Daily Steps Breakdown</Text>
        </View>
        <View style={fitnessStyles.chartWrapper}>
          {dailyStepsData.values.some(v => v > 0) ? (
            <ScrollableChart dataLength={dailyStepsData.values.length} visibleWidth={chartWidth}>
              {computedWidth => (
                <BarChart
                  values={dailyStepsData.values}
                  labels={dailyStepsData.labels}
                  width={computedWidth}
                  height={180}
                  maxY={stepsMax}
        ...
        <Text style={fitnessStyles.caption}>Total daily steps across the week.</Text>
        <View style={fitnessStyles.summaryMetricsRow}>
          <Text style={fitnessStyles.summaryMetricTarget}>Target: {stepsChartsMeta.target}</Text>
          <Text style={fitnessStyles.summaryMetricActual}>Actual: {stepsChartsMeta.actual}</Text>
          <Text style={fitnessStyles.summaryMetricPerformance}>Performance: {stepsChartsMeta.performance}%</Text>
        </View>
      </View>

      {/* Daily Heart Points Card */}
      <View style={fitnessStyles.card}>
        <View style={fitnessStyles.cardTitleRow}>
          <View style={[fitnessStyles.cardIconContainer, { backgroundColor: DC.roseLight }]}>
            <Text style={fitnessStyles.emptyChartIcon}>❤️</Text>
          </View>
          <Text style={fitnessStyles.cardTitle}>Daily Heart Points Breakdown</Text>
        </View>
        <View style={fitnessStyles.chartWrapper}>
          {dailyHeartPointsData.values.some(v => v > 0) ? (
            <ScrollableChart dataLength={dailyHeartPointsData.values.length} visibleWidth={chartWidth}>
              {computedWidth => (
                <BarChart
                  values={dailyHeartPointsData.values}
                  labels={dailyHeartPointsData.labels}
                  width={computedWidth}
                  height={180}
                  maxY={heartPointsMax}
        ...
        <Text style={fitnessStyles.caption}>Heart point progress for each day.</Text>
        <View style={fitnessStyles.summaryMetricsRow}>
          <Text style={fitnessStyles.summaryMetricTarget}>Target: {heartPointsChartsMeta.target}</Text>
          <Text style={fitnessStyles.summaryMetricActual}>Actual: {heartPointsChartsMeta.actual}</Text>
          <Text style={fitnessStyles.summaryMetricPerformance}>Performance: {heartPointsChartsMeta.performance}%</Text>
        </View>
      </View>

      {/* Energy Expended Card */}
      <View style={fitnessStyles.card}>
        <View style={fitnessStyles.cardTitleRow}>
          <View style={[fitnessStyles.cardIconContainer, { backgroundColor: DC.roseLight }]}>
            <Text style={fitnessStyles.emptyChartIcon}>🔥</Text>
          </View>
          <Text style={fitnessStyles.cardTitle}>Energy Expended</Text>
        </View>
        <View style={fitnessStyles.chartWrapper}>
          {energyExpendedData.values.some(v => v > 0) ? (
            <ScrollableChart dataLength={energyExpendedData.values.length} visibleWidth={chartWidth}>
              {computedWidth => (
                <SingleLineChart
                  values={energyExpendedData.values}
                  labels={energyExpendedData.labels}
                  width={computedWidth}
                  height={180}
                  color="#EF4444"
                  filled={true}
        ...
        <Text style={fitnessStyles.caption}>Energy expended performance across the week.</Text>
        <View style={fitnessStyles.summaryMetricsRow}>
          <Text style={fitnessStyles.summaryMetricTarget}>Target: {energyChartsMeta.target}</Text>
          <Text style={fitnessStyles.summaryMetricActual}>Actual: {energyChartsMeta.actual}</Text>
          <Text style={fitnessStyles.summaryMetricPerformance}>Performance: {energyChartsMeta.performance}%</Text>
        </View>
      </View>
      ...
      <View style={fitnessStyles.card}>
        <View style={fitnessStyles.cardTitleRow}>
          <View style={[fitnessStyles.cardIconContainer, { backgroundColor: DC.tealLight }]}>
            <Text style={fitnessStyles.emptyChartIcon}>⚡</Text>
          </View>
          <Text style={fitnessStyles.cardTitle}>Daily S-DEX Activity Breakdown</Text>
        </View>
        <View style={fitnessStyles.chartWrapper}>
          {sdexActivityData.values.some(v => v > 0) ? (
            <ScrollableChart dataLength={sdexActivityData.values.length} visibleWidth={chartWidth}>
              {computedWidth => (
                <BarChart
                  values={sdexActivityData.values}
                  labels={sdexActivityData.labels}
                  width={computedWidth}
                  height={180}
                  maxY={sdexMax}
        ...
        <Text style={fitnessStyles.caption}>S-DEX activity trend across the week.</Text>
        <View style={fitnessStyles.summaryMetricsRow}>
          <Text style={fitnessStyles.summaryMetricTarget}>Target: {sdexChartsMeta.target}</Text>
          <Text style={fitnessStyles.summaryMetricActual}>Actual: {sdexChartsMeta.actual}</Text>
          <Text style={fitnessStyles.summaryMetricPerformance}>Performance: {sdexChartsMeta.performance}%</Text>
        </View>
      </View>
    </>
  );
